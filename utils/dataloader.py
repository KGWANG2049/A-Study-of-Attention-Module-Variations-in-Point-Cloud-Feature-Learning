...
def get_shapenet_dataset_AnTao350M(saved_path, selected_points, fps_enable, augmentation, num_aug, jitter, std, clip,
                                   rotate, which_axis,
                                   angle_range, translate, x_translate_range, y_translate_range, z_translate_range,
                                   anisotropic_scale,
                                   x_scale_range, y_scale_range, z_scale_range):
    # get dataset
    train_set = ShapeNet_AnTao350M(saved_path, 'train', selected_points, fps_enable, augmentation, num_aug, jitter, std,
                                   clip, rotate, which_axis,
                                   angle_range, translate, x_translate_range, y_translate_range, z_translate_range,
                                   anisotropic_scale,
                                   x_scale_range, y_scale_range, z_scale_range)
    validation_set = ShapeNet_AnTao350M(saved_path, 'val', selected_points, fps_enable, False, num_aug, jitter, std,
                                        clip, rotate, which_axis,
                                        angle_range, translate, x_translate_range, y_translate_range, z_translate_range,
                                        anisotropic_scale,
                                        x_scale_range, y_scale_range, z_scale_range)
    trainval_set = ShapeNet_AnTao350M(saved_path, 'trainval', selected_points, fps_enable, augmentation, num_aug,
                                      jitter, std, clip, rotate, which_axis,
                                      angle_range, translate, x_translate_range, y_translate_range, z_translate_range,
                                      anisotropic_scale,
                                      x_scale_range, y_scale_range, z_scale_range)
    test_set = ShapeNet_AnTao350M(saved_path, 'test', selected_points, fps_enable, False, num_aug, jitter, std, clip,
                                  rotate, which_axis,
                                  angle_range, translate, x_translate_range, y_translate_range, z_translate_range,
                                  anisotropic_scale,
                                  x_scale_range, y_scale_range, z_scale_range)
...
class ModelNet_Alignment1024(torch.utils.data.Dataset):
    def __init__(self, saved_path, partition, selected_points, fps_enable, augmentation, num_aug, jitter, std, clip,
                 rotate, angle_sigma,
                 angle_clip, translate, x_translate_range, y_translate_range, z_translate_range, anisotropic_scale,
                 x_scale_range, y_scale_range, z_scale_range):
        ...
        if augmentation:
            self.augmentation_list = []
            if jitter:
                self.augmentation_list.append([data_augmentation.jitter_with_normV, [std, clip]])
            if rotate:
                self.augmentation_list.append([data_augmentation.rotate_with_normV, [angle_sigma, angle_clip]])
            if translate:
                self.augmentation_list.append(
                    [data_augmentation.translate_with_normV, [x_translate_range, y_translate_range, z_translate_range]])
            if anisotropic_scale:
                self.augmentation_list.append(
                    [data_augmentation.anisotropic_scale_with_normV, [x_scale_range, y_scale_range, z_scale_range]])
            if not jitter and not rotate and not translate and not anisotropic_scale:
                raise ValueError('At least one kind of data augmentation should be applied!')
            if len(self.augmentation_list) < num_aug:
                raise ValueError(
        ...
        xyz = self.all_pcd[:, :, :3]
        normalized_xyz = normalize_data(xyz)
        self.all_pcd = np.concatenate((normalized_xyz, self.all_pcd[:, :, 3:]), axis=2)
...
def get_modelnet_dataset_Alignment1024(saved_path, selected_points, fps_enable, augmentation, num_aug, jitter, std,
                                       clip, rotate, angle_sigma,
                                       angle_clip, translate, x_translate_range, y_translate_range, z_translate_range,
                                       anisotropic_scale,
                                       x_scale_range, y_scale_range, z_scale_range):
    # get dataset
    trainval_set = ModelNet_Alignment1024(saved_path, 'trainval', selected_points, fps_enable, augmentation, num_aug,
                                          jitter, std, clip, rotate, angle_sigma,
                                          angle_clip, translate, x_translate_range, y_translate_range,
                                          z_translate_range, anisotropic_scale,
                                          x_scale_range, y_scale_range, z_scale_range)
    test_set = ModelNet_Alignment1024(saved_path, 'test', selected_points, fps_enable, False, num_aug, jitter, std,
                                      clip, rotate, angle_sigma,
                                      angle_clip, translate, x_translate_range, y_translate_range, z_translate_range,
                                      anisotropic_scale,
                                      x_scale_range, y_scale_range, z_scale_range)
...
class ShapeNet_Normal(torch.utils.data.Dataset):
    def __init__(self, root, json_path, mapping, selected_points, fps_enable, augmentation, num_aug, jitter, std, clip,
                 rotate, which_axis,
                 angle_range, translate, x_translate_range, y_translate_range, z_translate_range, anisotropic_scale,
                 x_scale_range, y_scale_range, z_scale_range):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        pcd_path = os.path.join(self.root, category_hash, f'{pcd_hash}.txt')
        pcd_data = np.loadtxt(pcd_path)
        print(pcd_data.shape)
        pcd = pcd_data[:, 0:6]
        xyz_to_normalize = pcd[:, :3]
        xyz_normalized = pc_normalize(xyz_to_normalize)

        pcd[:, :3] = xyz_normalized
        pcd_labels = pcd_data[:, 6].astype(np.int64)

        if self.fps_enable:
            if self.selected_points <= len(pcd):
                pcd = torch.Tensor(pcd[None, ...]).cuda()
                pcd, indices = fps(pcd, K=self.selected_points, random_start_point=True)
                pcd, indices = pcd[0].cpu().numpy(), indices[0].cpu().numpy()
                pcd_labels = pcd_labels[indices]
            else:
        ...
        pcd = torch.Tensor(pcd).to(torch.float32).permute(1, 0)
        pcd_labels = torch.tensor(pcd_labels, dtype=torch.int64)
        seg_label = F.one_hot(pcd_labels, 50).to(torch.float32).permute(1, 0)
        category_id = self.mapping[category_hash]['category_id']
        category_onehot = F.one_hot(torch.tensor([category_id]), num_classes=16).to(torch.float32).permute(1, 0)
...
def get_shapenet_dataset_Yi650M(saved_path, mapping, selected_points, fps_enable, augmentation, num_aug, jitter,
                                ...
                                z_scale_range):
    dataset_path = Path(saved_path, 'shapenetcore_partanno_segmentation_benchmark_v0_normal')
    ...
    # get datasets
    train_set = ShapeNet_Normal(dataset_path, [train_json], mapping, selected_points, fps_enable, augmentation, num_aug,
                                jitter,
                                std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                y_translate_range, z_translate_range, anisotropic_scale, x_scale_range, y_scale_range,
                                z_scale_range)
    validation_set = ShapeNet_Normal(dataset_path, [validation_json], mapping, selected_points, fps_enable, False,
                                     num_aug, jitter,
                                     std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                     y_translate_range, z_translate_range, anisotropic_scale, x_scale_range,
                                     y_scale_range,
                                     z_scale_range)
    trainval_set = ShapeNet_Normal(dataset_path, [train_json, validation_json], mapping, selected_points, fps_enable,
                                   augmentation, num_aug, jitter,
                                   std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                   y_translate_range, z_translate_range, anisotropic_scale, x_scale_range,
                                   y_scale_range,
                                   z_scale_range)
    test_set = ShapeNet_Normal(dataset_path, [test_json], mapping, selected_points, fps_enable, False, num_aug, jitter,
                               std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                               y_translate_range, z_translate_range, anisotropic_scale, x_scale_range, y_scale_range,
                               z_scale_range)
...
def get_shapenet_dataset_Yi650M(saved_path, mapping, selected_points, fps_enable, augmentation, num_aug, jitter,
    ...
    # get datasets
    train_set = ShapeNet_Yi650M(dataset_path, [train_json], mapping, selected_points, fps_enable, augmentation, num_aug,
                                jitter,
                                std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                y_translate_range, z_translate_range, anisotropic_scale, x_scale_range, y_scale_range,
                                z_scale_range)
    validation_set = ShapeNet_Yi650M(dataset_path, [validation_json], mapping, selected_points, fps_enable, False,
                                     num_aug, jitter,
                                     std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                     y_translate_range, z_translate_range, anisotropic_scale, x_scale_range,
                                     y_scale_range,
                                     z_scale_range)
    trainval_set = ShapeNet_Yi650M(dataset_path, [train_json, validation_json], mapping, selected_points, fps_enable,
                                   augmentation, num_aug, jitter,
                                   std, clip, rotate, which_axis, angle_range, translate, x_translate_range,
                                   y_translate_range, z_translate_range, anisotropic_scale, x_scale_range,
                                   y_scale_range,
